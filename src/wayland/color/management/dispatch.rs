use std::{os::unix::prelude::AsFd, sync::Mutex};
...
use super::{
    ...
    TransferFunction, wp_color_management_output_v1, wp_color_management_surface_v1, wp_color_manager_v1,
                    ...
                    if let Some(data) = image_description.data::<ImageDescriptionData>() {
                        compositor::with_states(&surface, |states| {
                            *states
                                .cached_state
                                .get::<ColorManagementSurfaceCachedState>()
                                .pending() = ColorManagementSurfaceCachedState {
            ...
            wp_image_description_v1::Request::GetInformation => {
                ...
                match &data.info.0.contents {
                    super::ImageDescriptionContents::ICC(IccData { data, file }) => {
                        ...
                        if file.is_none() {
                            match SealedFile::with_data(c"icc", data) {
                                Ok(new_file) => {
                                    *file = Some(new_file);
                                }
                                Err(err) => {
        ...
        match request {
            wp_image_description_creator_icc_v1::Request::SetIccFile {
                ...
                let Some(data) = data_guard.as_mut() else {
                    resource.post_error(
                        wp_image_description_creator_icc_v1::Error::AlreadyUsed,
                        "Creator was already used",
                    );
                    return;
                };
    ...
    fn request(
        ...
        match request {
            wp_image_description_creator_params_v1::Request::SetTfCicp { tf_code } => {
                ...
                let Some(data) = data_guard.as_mut() else {
                    resource.post_error(
                        wp_image_description_creator_params_v1::Error::AlreadyUsed,
                        "Creator was already used",
                    );
                    return;
                };
